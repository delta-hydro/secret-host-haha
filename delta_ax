...
function Verify()
    ...
    local response = request({
        Url = "https://api-gateway.platoboost.com/v1/public/whitelist/8/" .. game:GetService("Players").LocalPlayer.UserId .. "?s",
        Method = "GET"
    })
        ...
        if string.find(response.Body, "true") then
            ...
            if (#key > 0) then
                local redeemResponse = request({
                    Url = "https://api-gateway.platoboost.com/v1/authenticators/redeem/8/" .. game:GetService("Players").LocalPlayer.UserId .. "/" .. key,
                    Method = "POST"
                });
